...
#initialize model
class ABM_Model:
    def __init__(self, ticks, N_hh, N_ind, decision, mig_util, mig_threshold, wealth_factor, ag_factor, comm_scale, shock_method, jobs_avail, network_type, w1, w2, w3, k, threshold):
        ...
        for i in range(self.num_hh):
            a = Household(self.wealth_factor, self.ag_factor, self.w1, self.w2, self.w3, self.k, self.threshold)
        ...
        for i in self.hh_set['household']: #set network
            i.set_network(self.hh_set, self.network)
    ...
    def model_step(self): #model step does each
        ...
        for i in random_sched_hh: #these are the steps at each tick for hh
            ...
            agent_var[0].check_network(self.hh_set)
            agent_var[0].sum_utility(self.individual_set)
            agent_var[0].migrate(self.decision, self.individual_set, self.mig_util, self.mig_threshold, self.origin_comm, self.av_wealth, self.av_land)
    ...
    def generate_network(self): #create community level network with networkx 
        if self.network_type == 'random':
            pass
        if self.network_type == 'none':
            pass
        if self.network_type == 'small_world':
            self.network = nx.watts_strogatz_graph(self.num_hh, self.network_size, 0.15)
        if self.network_type == 'preferential':
            self.network = nx.barabasi_albert_graph(self.num_hh, self.network_size, 0.15)
    ...
    def average_wealth(self):
        ...
        self.av_wealth = self.av_wealth / self.num_hh
        if self.av_wealth == 0: #this is to prevent 0 divisions
            self.av_wealth = 1
        
    def average_land(self):
        self.av_land = 0
        for h in self.hh_set['household']:
            self.av_land += h.land_owned
        self.av_land = self.av_land / self.num_hh
